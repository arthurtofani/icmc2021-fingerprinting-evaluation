...
def file_precompute_peaks_or_hashes(analyzer, filename, precompdir,
    ...
    rr = root.split('/')[-1]
    opfname = os.path.join(precompdir, rr + precompext)
...
def do_cmd(cmd, analyzer, hash_tab, filename_iter, matcher, outdir, type, report, skip_existing=False, strip_prefix=None):
    ...
    if cmd == 'merge' or cmd == 'newmerge':
        ...
        for num, filename in enumerate(filename_iter):
            msgs, out2 = matcher.file_match_to_msgs(analyzer, hash_tab, filename, num)
            out += out2
            report(msgs, out)
...
def getusr(tx):
    return tx.split(".")[0].split("/")[-1].split("_")[0]
...
def setup_reporter(args):
    ...
    if opfile and len(opfile):
        f = open(opfile, "w")
        f2 = open(opfile + '.clean.csv', "w")
        def report(msglist, out=None):
            ...
            if out is None:
                ...
                for msg in out:
                    usr1 = getusr(msg[0])
                    usr2 = getusr(msg[1])
                    same_usr = int(usr1 == usr2)
                    t_diff = abs(msg[3] - msg[4])
                    msg2 = msg + [same_usr, t_diff]
                    #import code; code.interact(local=dict(globals(), **locals()))
                    m = ';'.join([str(x) for x in msg2])
                    f.write(m + "\n")
                    #if t_diff < 0.1 and same_usr == 0:
                    f2.write(m + "\n")
    else:
        def report(msglist, out=None):
