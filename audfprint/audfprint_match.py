...
class Matcher(object):
    ...
    def match_file(self, analyzer, ht, filename, number=None):
        ...
        return self.match_file_hashes(q_hashes, analyzer, ht, number=number)

    def match_file_hashes(self, q_hashes, analyzer, ht, number=None):
    ...
    def file_match_to_msgs(self, analyzer, ht, qry, number=None):
        ...
        if len(rslts) == 0:
            ...
            for (tophitid, nhashaligned, aligntime, nhashraw, rank,
                ...
                src_file = qry
                dest_file = ht.names[tophitid]
                duration = (max_time - min_time) * t_hop
                src_start_time = min_time * t_hop
                dest_start_time = (min_time + aligntime) * t_hop
                num_hashes = nhashaligned
                total_hashes = nhashraw


                if src_file != dest_file:
                    out2.append([src_file, dest_file, duration, src_start_time, dest_start_time, num_hashes, total_hashes])
